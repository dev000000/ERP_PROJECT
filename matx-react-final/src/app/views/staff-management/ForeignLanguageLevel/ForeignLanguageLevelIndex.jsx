...
import { LOAI_NGON_NGU_OPTIONS } from "app/utils/constant-project";
...
export default observer(function ForeignLanguageLevelIndex() {
  const { foreignLanguageLevelStore } = useStore();
  ...
  const schema = useMemo(
    () => ({
      idKey: "MaTDNN",
      field1: "LoaiNgoaiNgu",
      field2: "CapDoNgoaiNgu",
      idText: "Mã Trình Độ",
      text1: "Loại Ngoại Ngữ",
      text2: "Cấp độ ngoại ngữ",
      tempPrefix: TEMP_PREFIX,
      optionsField1: LOAI_NGON_NGU_OPTIONS,

      api: {
        list: () => foreignLanguageLevelStore.getForeignLanguageLevelList(),
        getList: () => foreignLanguageLevelStore.foreignLanguageLevelList || [],
        create: (payload) => foreignLanguageLevelStore.addForeignLanguageLevel(payload),
        update: (id, payload) => foreignLanguageLevelStore.updateForeignLanguageLevel(id, payload),
        remove: (id) => foreignLanguageLevelStore.deleteForeignLanguageLevel(id),

        // them row tam
        pushTempRow: (row) => {
          runInAction(() => {
            foreignLanguageLevelStore.foreignLanguageLevelList = [
              row,
              ...(foreignLanguageLevelStore.foreignLanguageLevelList || [])
            ];
          });
  ...
  const processRowUpdate = async (newRow) => {
    ...
    if (!newRow?.[schema.field2]?.trim()) {
      throw new Error(`Không được để trống cột ${schema.text2}`);
    }
    if (
      existsByFields(foreignLanguageLevelStore.foreignLanguageLevelList, newRow, [
        "LoaiNgoaiNgu",
        "CapDoNgoaiNgu"
      ])
    ) {
      throw new Error("Loại ngoại ngữ và cấp độ ngoại ngữ này đã có rồi!");
    }
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        editable: true,
        type: "singleSelect",
        valueOptions: schema.optionsField1
      },
      {
        field: schema.field2,
        ...
        <DialogContent dividers>
          <Typography variant="body2">
            ...
            {deletingRow?.[schema.field1] ? (
              <>
                {" "}
                “<b>{`${deletingRow?.[schema.field1]}-${deletingRow?.[schema.field2]}`}</b>”
              </>
            ) : (
              " bản ghi này"
            )}
