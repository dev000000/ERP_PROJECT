...
export default observer(function SalaryRankIndex() {
  const { salaryRankStore, salaryGradeStore } = useStore();
  ...
  const schema = useMemo(
    () => ({
      ...
      optionsField1: null,
      optionField3: salaryGradeStore.salaryGradeList,

      api: {
        optionList: () => salaryGradeStore.getSalaryGradeList(),
        getOptionList: () => salaryGradeStore.salaryGradeList || [],
        list: () => salaryRankStore.getSalaryRankList(),
        getList: () => salaryRankStore.salaryRankList || [],
        create: (payload) => salaryRankStore.addSalaryRank(payload),
        update: (id, payload) => salaryRankStore.updateSalaryRank(id, payload),
        remove: (id) => salaryRankStore.deleteSalaryRank(id),

        // them row tam
        pushTempRow: (row) => {
          runInAction(() => {
            salaryRankStore.salaryRankList = [row, ...(salaryRankStore.salaryRankList || [])];
          });
        },
        // xoa row tam theo id
        removeTempRow: (id) => {
          runInAction(() => {
            salaryRankStore.salaryRankList = (salaryRankStore.salaryRankList || []).filter(
              (r) => r?.[schema.idKey] !== id
            );
          });
        }
  ...
  useEffect(() => {
    if (schema.api.getOptionList().length === 0) {
      schema.api.optionList();
    }
    schema.api.list();
  }, [schema]);
  ...
  // lay raw list tu store
  const gradeRaw = schema.api.getOptionList();
  // chuyen thanh mang {value, label} cho singleSelect
  const gradeOptions = useMemo(
    () =>
      (gradeRaw || []).map((g) => ({
        value: g.MaNgach,
        label: `${g.MaNgach} - ${g.TenNgach}`
      })),
    [gradeRaw]
  );

  // Map nhanh de hien thi label (neu muon format tay)
  const gradeLabelByValue = useMemo(() => {
    const m = new Map();
    gradeOptions.forEach((o) => m.set(o.value, o.label));
    return m;
  }, [gradeOptions]);
  ...
  const processRowUpdate = async (newRow) => {
    ...
    if (isNaN(Number(newRow?.[schema.field2]))) {
      throw new Error(`${schema.text2} phải là số hợp lệ`);
    }
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        minWidth: 160,
        editable: true,
        ...(schema.optionsField1 ? { valueOptions: schema.optionsField1 } : {})
      },
      {
        field: schema.field2,
      ...
      },
      {
        ...
        editable: true,
        type: "singleSelect",
        valueOptions: gradeOptions,
        valueFormatter: ({ value }) => gradeLabelByValue.get(value) ?? value
      },
      {
        field: "actions",
        type: "actions",
