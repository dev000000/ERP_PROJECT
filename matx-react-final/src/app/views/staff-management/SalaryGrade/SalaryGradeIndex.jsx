...
export default observer(function SalaryGradeIndex() {
  const { salaryGradeStore } = useStore();
  ...
  const schema = useMemo(
    () => ({
      ...
      api: {
        list: () => salaryGradeStore.getSalaryGradeList(),
        getList: () => salaryGradeStore.salaryGradeList || [],
        create: (payload) => salaryGradeStore.addSalaryGrade(payload),
        update: (id, payload) => salaryGradeStore.updateSalaryGrade(id, payload),
        remove: (id) => salaryGradeStore.deleteSalaryGrade(id),

        // them row tam
        pushTempRow: (row) => {
          runInAction(() => {
            salaryGradeStore.salaryGradeList = [row, ...(salaryGradeStore.salaryGradeList || [])];
          });
        },
        // xoa row tam theo id
        removeTempRow: (id) => {
          runInAction(() => {
            salaryGradeStore.salaryGradeList = (salaryGradeStore.salaryGradeList || []).filter(
              (r) => r?.[schema.idKey] !== id
            );
          });
        }
  ...
  const processRowUpdate = async (newRow) => {
    ...
    if (existsByFields(salaryGradeStore.salaryGradeList, newRow, ["TenNgach", "NgachLuong"])) {
      throw new Error("Tên ngạch và ngạch lương này đã có rồi!");
    }
