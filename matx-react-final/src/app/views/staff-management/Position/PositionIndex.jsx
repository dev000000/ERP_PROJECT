...
import { TRANG_THAI_CHUC_VU_OPTIONS } from "app/utils/constant-project";
...
export default observer(function PositionIndex() {
  ...
  const schema = useMemo(
    () => ({
      ...
      optionsField2: TRANG_THAI_CHUC_VU_OPTIONS,

      api: {
        list: () => positionStore.getPositionList(),
        getList: () => positionStore.positionList || [],
        create: (payload) => positionStore.addPosition(payload),
        update: (id, payload) => positionStore.updatePosition(id, payload),
        remove: (id) => positionStore.deletePosition(id),

        // them row tam
        pushTempRow: (row) => {
          runInAction(() => {
            positionStore.positionList = [row, ...(positionStore.positionList || [])];
          });
        },
        // xoa row tam theo id
        removeTempRow: (id) => {
          runInAction(() => {
            positionStore.positionList = (positionStore.positionList || []).filter(
              (r) => r?.[schema.idKey] !== id
            );
          });
        }
  ...
  const processRowUpdate = async (newRow) => {
    ...
    if (
      existsByFields(positionStore.positionList, newRow, ["TenChucVu"])
    ) {
      throw new Error("Tên chức vụ này đã có rồi!");
    }
